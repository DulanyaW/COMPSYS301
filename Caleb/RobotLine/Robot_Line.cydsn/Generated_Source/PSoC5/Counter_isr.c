...
{
    #ifdef Counter_isr_INTERRUPT_INTERRUPT_CALLBACK
        Counter_isr_Interrupt_InterruptCallback();
    #endif /* Counter_isr_INTERRUPT_INTERRUPT_CALLBACK */ 
...
void Counter_isr_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)Counter_isr__INTC_NUMBER] = address;
}
...
cyisraddress Counter_isr_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)Counter_isr__INTC_NUMBER];
}
...
*
*   Note calling Counter_isr_Start or Counter_isr_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after Counter_isr_Start or Counter_isr_StartEx has been called. 
...
{
    *Counter_isr_INTC_PRIOR = priority << 5;
}
...
uint8 Counter_isr_GetPriority(void)
{
    ...
    priority = *Counter_isr_INTC_PRIOR >> 5;
